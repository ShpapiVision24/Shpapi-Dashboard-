...
ig_uid, _page_id, page_token, acct_error = fetch_ig_account()
media_list = []
if acct_error:
    st.warning(f"Instagram account lookup: {acct_error}")
elif ig_uid:
    media_list, _me = fetch_ig_recent_media(ig_uid, page_token)
    if not media_list:
...
with pick_col:
    if not media_list:
        ...
        _cur = st.session_state.get("boost_media_id")
        n_cols = 3
        for row_start in range(0, min(len(media_list), 12), n_cols):
            ...
            for g_col, item in zip(g_cols, row_items):
                ...
                with g_col:
                    ...
                    # Button click updates session_state — right_col reads it below
                    if st.button("✓" if is_sel else "Select", key=f"bp_{mid}", use_container_width=True):
                        st.session_state["boost_media_id"] = mid

# ── RIGHT: preview + form ─────────────────────────────────────────────────────
# Read session_state HERE — after pick_col has already processed any button click this cycle
with right_col:
...
""", unsafe_allow_html=True)
        ...
        img_html = (f"<img src='{prev_thumb}' style='width:100%;display:block;max-height:280px;object-fit:cover;' />"
                    if prev_thumb else '<div style="width:100%;height:180px;background:#f0f0f0;"></div>')
        st.markdown(f"""
